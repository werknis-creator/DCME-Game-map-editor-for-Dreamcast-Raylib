...
Nowości v2.5:
- Dodano obiekty low-poly: dumpster, cone, npc, hydrant, barrel, bench
- Usunięto rysowanie linii (RL_LINES) dla kompatybilności z PVR Dreamcast
- Zoptymalizowano wodę i ogień (mniej segmentów, mniej cząstek)
- Poprawiono wydajność (>=50 FPS przy 15 obiektach)
- Dodano occlusion culling (pomijanie obiektów dalej niż 80 jednostek)
- Limit FPS ustawiony na 100
***********************************************************************/
#include <kos.h>
#include <raylib.h>
#include <raymath.h>
...
#define MAX_MAPS 10
#define MAX_DRAW_DISTANCE 80.0f   // odległość cullingu
...
/* Słabe symbole dla map (generowane przez DCME) */
...
/* --- NOWE FUNKCJE RYSOWANIA (niskopoligonowe, identyczne z DCME) --- */
...
/* Ogień – zmniejszona liczba cząstek */
...
/* Mały domek – BEZ LINII, tylko trójkąty */
static void DrawSmallHouse(MapObject* obj) {
    ...
    rlEnd();
    rlPopMatrix();
    // LINIE USUNIĘTE – brak RL_LINES
}

/* Woda – zmniejszona liczba segmentów, jedno przełączenie backface */
...
/* NOWE OBIEKTY (identyczne z DCME) – poprawione DrawCylinderEx */
static void DrawDumpster(Vector3 pos, float s, float rot) {
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, pos.z);
    rlRotatef(rot, 0,1,0);
    rlScalef(s,s,s);
    DrawCube((Vector3){0,0.6f,0}, 1.6f,1.2f,1.0f, (Color){25,128,25,255});
    DrawCube((Vector3){0,1.25f,0}, 1.7f,0.1f,1.1f, DARKGRAY);
    rlPopMatrix();
}

static void DrawCone(Vector3 pos, float s, float rot) {
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, pos.z);
    rlRotatef(rot, 0,1,0);
    rlScalef(s,s,s);
    DrawCylinderEx((Vector3){0,0,0}, (Vector3){0,0.1f,0}, 0.4f, 0.4f, 8, DARKGRAY);
    DrawCylinderEx((Vector3){0,0.1f,0}, (Vector3){0,1.0f,0}, 0.3f, 0.0f, 8, ORANGE);
    rlPopMatrix();
}

static void DrawHydrant(Vector3 pos, float s, float rot) {
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, pos.z);
    rlRotatef(rot, 0,1,0);
    rlScalef(s,s,s);
    DrawCylinderEx((Vector3){0,0,0}, (Vector3){0,0.8f,0}, 0.25f, 0.25f, 8, RED);
    DrawSphereEx((Vector3){0,0.8f,0}, 0.3f, 6,8, RED);
    DrawCube((Vector3){0.25f,0.5f,0}, 0.15f,0.15f,0.3f, YELLOW);
    rlPopMatrix();
}

static void DrawBarrel(Vector3 pos, float s, float rot) {
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, pos.z);
    rlRotatef(rot, 0,1,0);
    rlScalef(s,s,s);
    DrawCylinderEx((Vector3){0,0,0}, (Vector3){0,1.2f,0}, 0.5f, 0.5f, 8, (Color){128,76,25,255});
    DrawCylinderEx((Vector3){0,0.1f,0}, (Vector3){0,0.15f,0}, 0.52f, 0.52f, 8, DARKGRAY);
    DrawCylinderEx((Vector3){0,1.05f,0}, (Vector3){0,1.1f,0}, 0.52f, 0.52f, 8, DARKGRAY);
    rlPopMatrix();
}

static void DrawBench(Vector3 pos, float s, float rot) {
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, pos.z);
    rlRotatef(rot, 0,1,0);
    rlScalef(s,s,s);
    DrawCube((Vector3){0,0.5f,0}, 2.0f,0.1f,0.5f, (Color){128,76,25,255});
    DrawCube((Vector3){-0.8f,0.25f,-0.15f}, 0.1f,0.5f,0.1f, DARKGRAY);
    DrawCube((Vector3){0.8f,0.25f,-0.15f}, 0.1f,0.5f,0.1f, DARKGRAY);
    DrawCube((Vector3){-0.8f,0.25f,0.15f}, 0.1f,0.5f,0.1f, DARKGRAY);
    DrawCube((Vector3){0.8f,0.25f,0.15f}, 0.1f,0.5f,0.1f, DARKGRAY);
    rlPopMatrix();
...
static void DrawMapObject(MapObject* obj) {
    ...
    case 6: break; // player1 – rysowany osobno
    case 10:
        DrawSmallHouse(obj);
        break;
    case 11:
    ...
    /* NOWE TYPY */
    case 12:
        DrawDumpster(obj->position, obj->scale, obj->rotation);
        break;
    case 13:
        DrawCone(obj->position, obj->scale, obj->rotation);
        break;
    case 14:
        DrawHuman(obj->position, 0.0f, obj->scale); // NPC
        break;
    case 15:
        DrawHydrant(obj->position, obj->scale, obj->rotation);
        break;
    case 16:
        DrawBarrel(obj->position, obj->scale, obj->rotation);
        break;
    case 17:
        DrawBench(obj->position, obj->scale, obj->rotation);
        break;
    default: break;
    }
}

/* --- Ładowanie mapy (dokładnie jak w main (2).c) --- */
...
static void DrawMap(void) {
    ...
    for (int i = 0; i < map_object_count; i++) {
        // Occlusion culling – pomijamy obiekty oddalone o więcej niż MAX_DRAW_DISTANCE
        float dist = Vector3Distance(camPos, map_objects[i].position);
...
int main(int argc, char *argv[]) {
    ...
    SetTargetFPS(100);   // Limit FPS do 100
    InitPlayer();
